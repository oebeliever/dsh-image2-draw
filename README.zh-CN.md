...
「设置 → 插件 → 插件配置」里可配置**多个端点**(最多 8 个),按列表顺序尝试;
某个端点因**网络失败 / 超时 / 401 / 402 / 403 / 404 / 429 / 5xx(含 524)** 失败时,
自动切到下一个端点。**HTTP 400 这类参数错误不会切换**(401/402/403/404/429 仍会切换)——
换端点也是同一个错。

- 每个端点可用**各自的密钥引用**(`IMAGE2_API_KEY`、`IMAGE2_API_KEY_2`…)与各自的 `model`;
- 失败过的端点会被"沉到队尾":下次请求直接从上一次成功的端点开始,不必每次重复撞坏的;
- 备用端点也挂掉时会自动试回主端点(自愈),无需手动恢复;
- 环境变量 `IMAGE2_NO_ROTATE=1` 可关闭该记忆,始终从第一个端点开始。

> ⚠️ 524/超时意味着**上游可能已经生成了图片**,此时切换会有重复计费的可能;
> **所有端点都失败时**,错误信息里会明确提示这一点;**若切换后成功拿到了图,则不会有额外提示** —— 请据此判断是否重新生成。

> ⚠️ **端点里的密钥一律用凭据引用(`apiKeyEnv`),不要写字段 `apiKey`。** 字面 `apiKey` 只存在于服务端、设置面板看不到也改不了;
> 而且**在面板里编辑一次端点列表就会把整个数组替换掉,手写的字面值随之消失**(之后生成会报"未配置密钥")。
> 需要多把密钥就多建几个凭据名:`IMAGE2_API_KEY`、`IMAGE2_API_KEY_2`…
