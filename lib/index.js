...
export function adaptiveSize(prompt, fallback) {
  ...
  if (PORTRAIT_WORDS.some(word => text.includes(word))) return '768x1024'
  if (LANDSCAPE_WORDS.some(word => text.includes(word))) return '1024x768'
  if (SQUARE_WORDS.some(word => text.includes(word))) return '1024x1024'
  return fallback
}

/** 解析尺寸：adaptive/别名/WxH，校验与 image2.py 相同。 */
export function resolveSize(value, prompt, fallback) {
  ...
  if (raw === 'adaptive' || raw === 'auto') raw = adaptiveSize(prompt, fallback)
...
export async function resolveKey(ctx, settings) {
  ...
  if (!credential) {
    throw new Error(`未配置密钥（credentialRef=${ref}），请到 设置 → 插件 → 插件配置 填写 API Key`)
  }
...
async function editOnce(ctx, settings, params, exec) {
  ...
  for (const part of parts) {
    form.append(field, new Blob([part.bytes], { type: part.mime }), part.name)
  }
...
export async function apply(ctx, config = {}, deps = {}) {
  ...
  const commonParameters = {
    ...
    size: { type: 'string', description: 'adaptive（默认，按提示词自动）、portrait、landscape、square，或 WIDTHxHEIGHT（两边 16 的倍数，最长边 ≤3840，总像素 655360~8294400，宽高比 ≤3:1）' },
    quality: { type: 'string', description: 'low / medium / high / auto；默认 low（同步代理约 120s 附近可能 524，高质量更慢）' },
  }
  ...
  ctx.tools.register(defineTool({
    name: 'image2-edit',
    description: '图生图：基于 1~8 张参考图调用配置好的 image2 API 修改/重绘。相对路径按会话工作目录解析；单张 ≤4MB、总计 ≤32MB，仅支持 PNG/JPEG/WebP（Node 端不自动缩放）。结果会显示在聊天卡片并保存到 outputs/image2。文件名由结果卡片展示，最终回复不要重复输出文件名。不要拼接完整路径，也不要调用读取图片或文件工具检查结果；当前模型可能不支持图片输入。',
    parameters: {
      ...commonParameters,
