...
import {
  ...
  // 多端点故障转移（core）：本文件此前未用过，漏导入即运行期 ReferenceError。
  resolveEndpoints, validateEndpoints, callWithFailover, readStartIndex, writeStartIndex, nextStartIndex,
} from './core.js'
...
const MUTABLE_SETTINGS_FIELDS = new Set(['baseURL', 'model', 'editURL', 'timeoutSeconds', 'endpoints'])
const SAFE_SETTINGS_FIELDS = ['apiKeyEnv', 'baseURL', 'model', 'editURL', 'timeoutSeconds', 'endpoints']
...
/**
 * settings 校验入口：core 的 validateSettings 只覆盖旧单端点字段，
 * 这里补上多端点数组（endpoints）的校验，再交给 installSettingsSection。
 */
function validateSettingsWithEndpoints(value) {
  validateSettings(value)
  const settings = objectOf(value)
  validateEndpoints(settings.endpoints)
}
...
async function generateOnce(ctx, settings, params, exec) {
  ...
  const outcome = await requestWithFailover(ctx, settings, async endpoint => {
    const model = endpoint.model ?? settings?.model ?? DEFAULT_MODEL
    const payload = buildGeneratePayload({ model }, params)
    const url = normalizeGenerationsUrl(endpoint.baseURL)
    const apiKey = await endpointKey(ctx, endpoint, settings)
  ...
  const file = saveBlob(outcome.value, outputBaseOf(exec), {})
  const image = await saveConversationImage(ctx, outcome.value, file)
  return {
    files: [file],
    images: image === undefined ? [] : [image],
    provider: 'image2',
    model: outcome.endpoint.model ?? settings?.model ?? DEFAULT_MODEL,
    size: params.size,
    quality: params.quality,
    endpoint: { name: outcome.endpoint.name, baseURL: outcome.endpoint.baseURL },
    failovers: outcome.attempts.length,
  }
}
...
async function editPartsOnce(ctx, settings, params, parts, exec) {
  ...
  const outcome = await requestWithFailover(ctx, settings, async endpoint => {
    const model = endpoint.model ?? settings?.model ?? DEFAULT_MODEL
    const url = editUrlOf({ baseURL: endpoint.baseURL, editURL: endpoint.editURL })
    const apiKey = await endpointKey(ctx, endpoint, settings)
    const form = buildEditForm({ model }, params, parts)
...
/** 端点起点指针的落盘位置(与 presets.json 同目录)。 */
export function endpointsStatePath() {
  return join(dshHomeOf(), 'storages', STUDIO_LIB_NAME, 'endpoints-state.json')
}

/** 解析某个端点的密钥：字面 apiKey 优先，否则走 credential 引用。 */
async function endpointKey(ctx, endpoint, settings) {
  const literal = String(endpoint?.apiKey ?? '').trim()
  if (literal !== '') return literal
  const ref = String(endpoint?.apiKeyEnv ?? settings?.apiKeyEnv ?? DEFAULT_KEY_ENV).trim() || DEFAULT_KEY_ENV
  if (!isCredentialRef(ref)) throw new Error(`apiKeyEnv「${ref}」不合法（须为环境变量式名称，如 IMAGE2_API_KEY）`)
  const credential = await ctx.credentials.resolve(ref)
  if (!credential) {
    throw new Error(`未配置密钥（credentialRef=${ref}）。请到 设置 → 插件 → 插件配置 填写 API Key；若你的中转按分组隔离模型（如仅 openai/Image 分组提供图片模型），请使用其 Image/图片 分组的密钥`)
...
/** 环形顺序 + 起点指针的故障转移外壳。 */
async function requestWithFailover(ctx, settings, attempt) {
  const endpoints = resolveEndpoints(settings)
  const rotate = String(process.env.IMAGE2_NO_ROTATE ?? '') !== '1'
  const statePath = endpointsStatePath()
  const start = rotate ? readStartIndex(statePath) : 0
  const outcome = await callWithFailover(endpoints, start, attempt, { rotate })
  if (rotate && outcome.usedIndex !== start) {
    writeStartIndex(statePath, nextStartIndex(start, outcome.usedIndex, endpoints.length))
  }
  return outcome
}
...
/** 端点行的对外视图：字面 apiKey 只在服务端使用（密钥请走 credential 引用），
 *  绝不随 /image2-draw/state 下发；apiKeyEnv 等其余字段原样保留。 */
function endpointRowForClient(entry) {
...
function safeSettingsValue(value) {
  ...
  for (const field of SAFE_SETTINGS_FIELDS) {
    ...
    safe[field] = field === 'endpoints' && Array.isArray(raw) ? raw.map(endpointRowForClient) : raw
  }
  return safe
}
...
function validateMutationBody(body) {
  ...
  for (const operation of body.ops) {
    ...
    if (field === 'endpoints') {
      // endpoints 是数组字段：走 core 的校验（消息自带 endpoints[i] 定位），不走下面的字符串路径。
      if (operation.value === undefined) throw new Error('字段 endpoints 必须提供数组值；清空请使用 unset')
      validateEndpoints(operation.value)
      continue
    }
...
export async function apply(ctx, config = {}, deps = {}) {
  ...
  installSettingsSection(ctx, NS, image2Schema, config, {
    ...
    onChange: () => {},
    validate: validateSettingsWithEndpoints,
  })
  ...
  ctx.tools.register(defineTool({
    ...
    description: '文生图：调用配置好的 image2 API（设置 → 插件 → 插件配置 统一配置 API Key 与接口地址）。结果会显示在聊天卡片并保存到会话工作目录 outputs/image2。文件名由结果卡片展示，最终回复不要重复输出文件名。不要拼接完整路径，也不要调用读取图片或文件工具检查结果；当前模型可能不支持图片输入。524/超时不会自动重试（上游可能仍在生成），如失败请把错误告诉用户确认后再试。支持多端点配置：某个端点因网络/超时/401/402/403/404/429/5xx 失败时会自动切换到下一个端点（400 类参数错误不会切换）；返回值里的 endpoint 是实际使用的端点、failovers 是切换次数。',
    parameters: {
      ...commonParameters,
