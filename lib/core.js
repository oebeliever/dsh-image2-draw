...
/** 故障转移判定：基础设施类失败切换；参数类 4xx 不切换(换端点也是同一个错)。
 *  无 HTTP 状态码的异常(连接失败 / DNS / 超时 / 响应非 JSON / 图片解码失败)一律视为基础设施类。 */
export function shouldFailover(error) {
  const status = Number(error?.status)
  if (!Number.isFinite(status)) return true
  if (status === 401 || status === 402 || status === 403 || status === 404 || status === 408) return true
  if (status === 429) return true
  if (status >= 500) return true
  return false
}

/** 成功后把起点指针移到"本次实际用到的端点"；未成功(usedIndex 非法)则保持原值。 */
export function nextStartIndex(prevStart, usedIndex, total) {
  const size = Number(total)
  const prev = Number.isInteger(prevStart) && prevStart >= 0 ? prevStart : 0
  if (!Number.isInteger(size) || size <= 0) return 0
  // null / undefined 表示"本轮没有任何端点成功",不是端点 0：Number(null) === 0 会把它当成合法下标。
  if (usedIndex === null || usedIndex === undefined) return prev % size
  const used = Number(usedIndex)
  if (!Number.isInteger(used) || used < 0 || used >= size) return prev % size
  return used
}
...
/**
 * 从 startIndex 起按环形顺序逐个尝试端点，直到成功或全部失败。
 * attempt(endpoint, index) 抛错即视为该端点失败；是否继续由 shouldFailover 决定 ——
 * 参数类错误(400/422 等)会立刻冒泡，不再尝试后续端点。
 */
export async function callWithFailover(endpoints, startIndex, attempt, options = {}) {
  if (!Array.isArray(endpoints) || endpoints.length === 0) throw new Error('没有可用端点')
  const total = endpoints.length
  const rotate = options.rotate !== false
  const start = Number.isInteger(startIndex) && startIndex >= 0 && startIndex < total ? startIndex : 0
  const attempts = []
  for (let offset = 0; offset < total; offset += 1) {
    const index = rotate ? (start + offset) % total : offset
    const endpoint = endpoints[index]
    try {
      const value = await attempt(endpoint, index)
      return { value, usedIndex: index, endpoint, attempts }
    } catch (error) {
      if (!shouldFailover(error)) throw error
      attempts.push({ index, name: endpoint.name, baseURL: endpoint.baseURL, error })
    }
  }
  const detail = attempts
    .map(item => {
      ...
      return `- ${label}${item.baseURL || '(未配置地址)'}：${message}`
    })
    .join('\n')
  const duplicated = attempts.some(item => item.error?.status === 524 || item.error?.name === 'AbortError')
  const hint = duplicated ? '\n提示：其中含 524/超时，上游可能已生成图片，存在重复计费可能。' : ''
  throw new AggregateError(
    attempts.map(item => item.error),
    `全部 ${total} 个端点均失败：\n${detail}${hint}`,
  )
}

/** 读起点指针；不存在 / 损坏 / 非法一律返回 0（状态是优化，不是正确性依赖）。 */
export function readStartIndex(statePath) {
  try {
    const raw = JSON.parse(readFileSync(statePath, 'utf8'))
    const index = Number(raw?.startIndex)
...
/** 写起点指针；自动建目录，任何失败静默忽略。 */
export function writeStartIndex(statePath, index) {
  try {
    mkdirSync(dirname(statePath), { recursive: true })
    writeFileSync(statePath, JSON.stringify({ startIndex: index }))
  } catch {
    // 忽略：下次请求退回从第一个端点开始，行为仍然正确
  }
}
