...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      endpointsLabel: '端点列表（按顺序尝试，失败自动切换）',
      endpointsHint: '每行一个端点；密钥引用名决定用哪个凭据（IMAGE2_API_KEY、IMAGE2_API_KEY_2…）。改动这个列表会写入 endpoints 配置。',
      endpointsInvalid: '端点配置不完整：地址不能为空且必须是 http(s)，密钥引用名须为字母/数字/下划线。',
      endpointAdd: '添加端点',
      endpointRemove: '删除',
      endpointNamePlaceholder: '名称（可选）',
      endpointModelPlaceholder: 'model（留空用全局）',
      endpointKeyPlaceholder: '密钥（留空不改）',
      endpointKeyPlaceholderSet: '密钥已配置（留空不改）',
    }
    ...
    const en = {
      ...
      endpointsLabel: 'Endpoints (tried in order, auto-failover)',
      endpointsHint: 'One endpoint per row; the credential reference picks which key to use (IMAGE2_API_KEY, IMAGE2_API_KEY_2, …). Editing this list writes the endpoints config.',
      endpointsInvalid: 'Incomplete endpoint config: baseURL must be a non-empty http(s) URL and the credential reference must be alphanumeric/underscore.',
      endpointAdd: 'Add endpoint',
      endpointRemove: 'Remove',
      endpointNamePlaceholder: 'Name (optional)',
      endpointModelPlaceholder: 'model (blank = global)',
      endpointKeyPlaceholder: 'API key (blank = keep)',
      endpointKeyPlaceholderSet: 'API key configured (blank = keep)',
    }
    ...
    const ENDPOINT_REF_RE = /^[A-Za-z_][A-Za-z0-9_]*$/

    /** settings → 端点草稿行（未配过 endpoints 时用旧的单端点字段合成一行）。 */
    function endpointDraftsOf(value) {
      const source = objectOf(value)
      const list = Array.isArray(source.endpoints)
        ? source.endpoints.filter(item => item !== null && typeof item === 'object' && !Array.isArray(item))
        : []
      const rows = list.length > 0 ? list : [source]
      return rows.map((item, index) => ({
        name: typeof item.name === 'string' ? item.name : '',
        baseURL: typeof item.baseURL === 'string' ? item.baseURL : '',
        model: typeof item.model === 'string' ? item.model : '',
        editURL: typeof item.editURL === 'string' ? item.editURL : '',
        apiKeyEnv: typeof item.apiKeyEnv === 'string' && item.apiKeyEnv.trim() !== ''
          ? item.apiKeyEnv.trim()
          : (index === 0 ? DEFAULT_KEY_ENV : `${DEFAULT_KEY_ENV}_${index + 1}`),
        apiKeyDraft: '',
        configured: false,
      }))
    }

    /** 草稿行 → endpoints 字段值（空字段丢弃；apiKeyEnv 必填）。 */
    function endpointsToValue(rows) {
      return rows.map(row => {
        const item = { baseURL: String(row.baseURL ?? '').trim() }
        const name = String(row.name ?? '').trim()
        const model = String(row.model ?? '').trim()
        const editURL = String(row.editURL ?? '').trim()
        if (name !== '') item.name = name
        if (model !== '') item.model = model
        if (editURL !== '') item.editURL = editURL
        item.apiKeyEnv = String(row.apiKeyEnv ?? '').trim() || DEFAULT_KEY_ENV
        return item
      })
    }

    /** 行级校验：地址合法且非空、editURL 合法或空、credential 名合法、数量在 1~8。 */
    function endpointsValid(rows) {
      if (!Array.isArray(rows) || rows.length < 1 || rows.length > MAX_ENDPOINTS) return false
      return rows.every(row => {
        const baseURL = String(row.baseURL ?? '').trim()
        if (baseURL === '' || !isValidBaseUrl(baseURL)) return false
        if (!isValidBaseUrl(row.editURL)) return false
        return ENDPOINT_REF_RE.test(String(row.apiKeyEnv ?? '').trim())
      })
    }
    ...
    function dirtyOf(settingsDirty, keyDraft, endpointsDirty) {
      return settingsDirty === true || String(keyDraft ?? '').trim() !== '' || endpointsDirty === true
    }
    ...
    function EndpointRow(props) {
      const { index, total, row, disabled, t, onEdit, onRemove, onMove } = props
      const invalidUrl = String(row.baseURL ?? '').trim() !== '' && !isValidBaseUrl(row.baseURL)
      return h('div', { className: 'dsh-i2-endpoint' },
        h('div', { className: 'dsh-i2-endpoint-head' },
          h('span', { className: 'dsh-i2-endpoint-index' }, `${index + 1}`),
          h('input', {
            className: 'dsh-i2-input',
            type: 'text',
            value: row.name,
        ...
        h('input', {
          className: invalidUrl ? 'dsh-i2-input dsh-i2-input-invalid' : 'dsh-i2-input',
          type: 'text',
          value: row.baseURL,
          placeholder: 'https://example.com/v1',
          disabled,
          onChange: event => onEdit(index, 'baseURL', event.target.value),
        }),
        invalidUrl ? h('p', { className: 'dsh-i2-invalid' }, t('invalidUrl')) : null,
        h('input', {
          className: 'dsh-i2-input',
          type: 'text',
          value: row.model,
          ...
          disabled,
          onChange: event => onEdit(index, 'model', event.target.value),
        }),
        h('div', { className: 'dsh-i2-endpoint-key' },
          h('input', {
            className: 'dsh-i2-input',
            type: 'password',
            autoComplete: 'new-password',
            spellCheck: false,
            name: `image2-api-key-${index}`,
            value: row.apiKeyDraft,
            placeholder: row.configured ? t('endpointKeyPlaceholderSet') : t('endpointKeyPlaceholder'),
            disabled,
            onChange: event => onEdit(index, 'apiKeyDraft', event.target.value),
          }),
          h('input', {
            className: 'dsh-i2-input',
            type: 'text',
            value: row.apiKeyEnv,
            placeholder: DEFAULT_KEY_ENV,
            disabled,
            onChange: event => onEdit(index, 'apiKeyEnv', event.target.value),
          }),
          h('span', { className: row.configured ? 'dsh-i2-badge dsh-i2-badge-ok' : 'dsh-i2-badge' },
            row.configured ? t('keySet') : t('keyUnset'))))
    }
    ...
    function Image2DrawForm(props) {
      ...
      const endpointsDirtyRef = useRef(false)
      ...
      const syncDirty = useCallback((settingsDirty, nextKeyDraft) => {
        ...
        const nextDirty = dirtyOf(settingsDirty, nextKeyDraft, endpointsDirtyRef.current)
        setDirty(nextDirty)
        dirtyRef.current = nextDirty
        onDirtyChange?.(nextDirty)
      }, [onDirtyChange])

      const describeEndpointKeys = async (value, rows) => {
        const refs = [...new Set(rows.map(row => row.apiKeyEnv).filter(ref => ref !== ''))]
        if (refs.length === 0) return
        try {
          const response = await api.credentials.describe({ refs })
          if (!response.result.ok) return
          const credentials = response.result.value.credentials ?? {}
          const legacyRef = keyRefOf(value)
          setKeyConfigured(credentials[legacyRef]?.configured === true)
          setKeyWritable(credentials[legacyRef]?.writable !== false)
          setEndpointRows(current => current.map(row => ({
            ...row,
            configured: credentials[row.apiKeyEnv]?.configured === true,
          })))
        } catch {
          // 保持未配置状态
        }
      ...
      const load = useCallback(async (preserveDrafts = false) => {
        ...
        try {
          ...
          if (!preserveDrafts) {
            const rows = endpointDraftsOf(state.value)
            setEndpointRows(rows)
            setDrafts(draftsOf(state.value))
            dirtyFieldsRef.current.clear()
            endpointsDirtyRef.current = false
            syncDirty(false, '')
            void describeEndpointKeys(state.value, rows)
          }
          setStatus('ready')
        } catch (error) {
      ...
      const editEndpointRow = (index, field, text) => {
        setEndpointRows(current => current.map((row, i) => (i === index ? { ...row, [field]: text } : row)))
        endpointsDirtyRef.current = true
        syncDirty(true, keyDraftRef.current)
        setNotice('')
      }

      const addEndpointRow = () => {
        setEndpointRows(current => {
          if (current.length >= MAX_ENDPOINTS) return current
          ...
          return [...current, {
            ...
            model: '',
            editURL: '',
            apiKeyEnv: index === 0 ? DEFAULT_KEY_ENV : `${DEFAULT_KEY_ENV}_${index + 1}`,
            apiKeyDraft: '',
            configured: false,
          }]
        })
      ...
      const removeEndpointRow = (index) => {
        setEndpointRows(current => (current.length <= 1 ? current : current.filter((_, i) => i !== index)))
        endpointsDirtyRef.current = true
        syncDirty(true, keyDraftRef.current)
        setNotice('')
      }

      const moveEndpointRow = (index, delta) => {
        setEndpointRows(current => {
      ...
      const save = async () => {
        ...
        if (endpointsDirtyRef.current && !endpointsValid(endpointRows)) return
        setBusy(true)
        setFailure('')
        setNotice('')
        ...
        try {
          if (plan.mutateSettings) {
            try {
              const rows = endpointsToValue(endpointRows)
              const first = rows[0] ?? {}
              const ops = endpointsDirtyRef.current
                ? [
                    { op: 'set', path: ['endpoints'], value: rows },
                    // 不 unset 旧字段,而是镜像第一个端点:可视化工作台(studio)仍读 settings.baseURL,
                    // 直接清掉会让工作台生成时报"未配置接口地址"。endpoints 仍是工具路径的唯一权威。
                    { op: 'set', path: ['baseURL'], value: first.baseURL ?? '' },
                    first.model !== undefined
                      ? { op: 'set', path: ['model'], value: first.model }
                      : { op: 'unset', path: ['model'] },
                    first.editURL !== undefined
                      ? { op: 'set', path: ['editURL'], value: first.editURL }
                      : { op: 'unset', path: ['editURL'] },
                    // 超时不在端点行里，但它和这次保存同生共死：漏掉这一条，同批次的超时草稿会被
                    // 后面的 clear 标成"已保存"、再被响应回填覆盖，用户看到的是一次无声回退。
                    ...opsFor(drafts, ['timeoutSeconds']),
                  ]
                : opsFor(drafts, dirtyFieldsRef.current)
          ...
          if (plan.writeKey) {
            try {
              const response = await api.credentials.set({ ref: keyRefOf(acceptedView.value), value: keyDraftRef.current.trim() })
          ...
          // settings 已落盘且 dirty 已被清过：密钥写失败的出口必须把列表重新标脏，
          // 否则保存按钮一直灰着，刚填进草稿的密钥再也提交不出去。
          const markKeyWritePending = () => {
            endpointsDirtyRef.current = true
            syncDirty(settingsDirtyRef.current, keyDraftRef.current)
          }
          for (const row of endpointRows) {
            const draft = String(row.apiKeyDraft ?? '').trim()
            if (draft === '') continue
            // 引用名必须与写进 settings 的那份完全同源（endpointsToValue 也做同样的 trim + 兜底），
            // 否则引用名带空格时会"配置指向甲、密钥写到乙"。
            const ref = String(row.apiKeyEnv ?? '').trim() || DEFAULT_KEY_ENV
            try {
          ...
          // 密钥都写完后，用服务端返回的 settings 重建草稿行（apiKeyDraft 随之清空）并重新 describe，
          // 让"已配置"徽标反映真实凭据状态，而不是一刀切成 true/false。
          const nextRows = endpointDraftsOf(acceptedView.value)
          setEndpointRows(nextRows)
          void describeEndpointKeys(acceptedView.value, nextRows)
          // 输入在保存期间禁用，因此成功后可在一个出口原子清空所有草稿标记。
          // 列表脏标记同样要在这里清：重试路径（上次 settings 已存、仅密钥写失败）里
          // plan.mutateSettings 为 false，mutate 分支不会再跑，漏掉这一行会让
          // markKeyWritePending 置的 true 永远悬着——保存成功了却一直显示"未保存"。
          // 此刻清它是安全的：mutateSettings 为 false 意味着 settings 侧的改动都已落盘。
          dirtyFieldsRef.current.clear()
          endpointsDirtyRef.current = false
          syncDirty(false, '')
          setNotice(t('saved'))
      ...
      return h('div', { className: 'dsh-i2-form' },
        ...
        h('div', { className: 'dsh-i2-endpoints' },
          h('div', { className: 'dsh-i2-field-head' },
            h('label', { className: 'dsh-i2-label' }, t('endpointsLabel')),
            h('button', {
              className: 'dsh-i2-secondary',
              type: 'button',
              disabled: !writable || busy || endpointRows.length >= MAX_ENDPOINTS,
              onClick: addEndpointRow,
            }, t('endpointAdd'))),
          h('p', { className: 'dsh-i2-hint' }, t('endpointsHint')),
          endpointRows.map((row, index) => h(EndpointRow, {
            key: `endpoint-${index}`,
            index,
            ...
            t,
            onEdit: editEndpointRow,
            onRemove: removeEndpointRow,
            onMove: moveEndpointRow,
          })),
          !endpointsValid(endpointRows) ? h('p', { className: 'dsh-i2-invalid' }, t('endpointsInvalid')) : null),
    ...
    /** 模板库：内置（版式A/B、逐套流程、着装清单）+ 用户自定义的存取与载入。 */
    function PresetBar(props) {
      ...
      const refresh = useCallback(async () => {
        try {
          const value = await fetchJson('/image2-draw/presets')
          setPresets(Array.isArray(value.items) ? value.items : [])
          setError('')
        } catch (loadError) {
      ...
      const applySelected = () => {
        ...
        const body = objectOf(item.body)
        if (item.kind === 'prompt') {
          const template = String(body.prompt ?? '')
          const variableList = Array.isArray(body.variables) ? body.variables : []
          const missing = variableList.filter(name => !String(vars[name] ?? '').trim())
          if (missing.length > 0) {
            setPending(item)
            return
          }
          ...
          for (const name of variableList) {
            text = text.split(`{${name}}`).join(String(vars[name]).trim())
          }
          onFormChange({
            mode: body.mode === 'generate' ? 'generate' : 'edit',
            size: typeof body.size === 'string' ? body.size : '1536x1024',
            quality: typeof body.quality === 'string' ? body.quality : 'high',
            prompt: text,
          })
          setNotice(`已载入「${item.name}」，可继续编辑后生成`)
        } else {
          setPending(undefined)
          setPreview(previewOf(item))
          setNotice(item.kind === 'flow'
            ? '流程模板：在对话中让 AI 用 image2-preset 工具读取并执行（会逐张等你确认）'
            : '清单模板：可复制后填入版式模板的 {outfit}，或让 AI 在对话中读取')
        }
      }

      const variableRows = pending !== undefined
        ? (Array.isArray(objectOf(pending.body).variables) ? objectOf(pending.body).variables : [])
            .map(name => h('div', { className: 'dsh-i2s-var-row', key: name },
              h('label', { className: 'dsh-i2s-param-label', htmlFor: `preset-var-${name}` }, `{${name}}`),
              h('input', {
                id: `preset-var-${name}`,
                className: 'dsh-i2s-input',
                type: 'text',
                placeholder: name === 'outfit' ? '该套穿着英文描述，如：black sleeveless athletic tank with red shoulder accents, charcoal cap worn backwards, dark athletic shorts, thin silver chain necklace' : `填写 ${name}`,
      ...
      const saveCurrent = async () => {
        ...
        if (String(form.prompt ?? '').trim() === '') {
          setError('当前没有可保存的提示词（请先填写画面描述或载入模板）')
          return
        }
        setBusy(true)
        setError('')
        setNotice('')
        try {
          const placeholders = [...new Set(String(form.prompt).match(/\{([A-Za-z0-9_]+)\}/g) ?? [])]
            .map(token => token.slice(1, -1))
          await fetchJson('/image2-draw/presets', {
            name,
            kind: 'prompt',
            desc: '工作台保存的自定义提示词模板',
            body: {
      ...
      const removeSelected = async () => {
        const item = selected
        if (item === undefined || item.builtin === true) return
        setBusy(true)
        setError('')
        try {
          const response = await fetch(`/image2-draw/presets?id=${encodeURIComponent(item.id)}`, { method: 'DELETE' })
          let payload
          try {
            payload = await response.json()
          } catch {
          ...
          if (response.status !== 200 || payload?.ok !== true) {
            throw new Error(payload?.error?.message ?? `删除失败（HTTP ${response.status}）`)
          }
          setSelectedId('')
          setPreview('')
          await refresh()
          setNotice(`已删除模板：「${item.name}」`)
        } catch (removeError) {
      ...
      return h('div', { className: 'dsh-i2s-presets' },
        h('div', { className: 'dsh-i2s-preset-head' },
          h('span', { className: 'dsh-i2s-section-label' }, '📚 模板库'),
          h('span', { className: 'dsh-i2s-hint' }, '内置：人物设计图版式A/B · 逐套确认流程 · 六套着装')),
        h('div', { className: 'dsh-i2s-preset-row' },
          h('select', {
            className: 'dsh-i2s-select',
            value: selectedId,
            onChange: event => {
            ...
            h('option', { value: '', disabled: true }, '选择模板…'),
            ...presets.map(item => h('option', { key: item.id, value: item.id },
              `${item.builtin ? '⭐' : '📁'} ${item.name}（${item.kind}）`))),
          h('button', {
            type: 'button',
            className: 'dsh-i2s-secondary',
            disabled: selected === undefined || busy,
            onClick: applySelected,
          }, '载入'),
          selected !== undefined && selected.builtin !== true
            ? h('button', {
                type: 'button',
                className: 'dsh-i2s-secondary',
                disabled: busy,
        ...
        variableRows.length > 0
          ? h('div', { className: 'dsh-i2s-var-box' },
              ...variableRows,
              h('div', { className: 'dsh-i2s-var-actions' },
                h('button', {
                  type: 'button',
                  className: 'dsh-i2s-magic',
                  onClick: () => {
                    setPending(undefined)
                    applySelected()
                  },
          ...
          : null,
        preview !== ''
          ? h('div', { className: 'dsh-i2s-preview' },
              h('textarea', {
                className: 'dsh-i2s-textarea',
                readOnly: true,
                rows: Math.min(8, Math.max(3, preview.split('\n').length)),
                value: preview,
                onChange: () => {},
              }),
              h('button', { type: 'button', className: 'dsh-i2s-secondary', onClick: copyPreview }, '复制全文'))
          : null,
        h('div', { className: 'dsh-i2s-preset-save' },
          h('input', {
            className: 'dsh-i2s-input',
            type: 'text',
            placeholder: '把当前提示词存为模板：输入名称',
            value: nameDraft,
        ...
        error !== '' ? h('p', { className: 'dsh-i2s-error', role: 'alert' }, error) : null,
        notice !== '' ? h('p', { className: 'dsh-i2s-success' }, notice) : null)
    }
    ...
    function Image2StudioPanel(props) {
      ...
      const handleFilesChange = useCallback((action) => {        if (action === null || typeof action !== 'object') return
    ...
    const css = `
      ...
      .dsh-i2-endpoints{display:flex;flex-direction:column;gap:10px;padding:12px;border:1px solid var(--dsh-border,rgba(127,127,127,.28));border-radius:8px}
      .dsh-i2-endpoint{display:flex;flex-direction:column;gap:6px;padding:10px;border:1px solid var(--dsh-border,rgba(127,127,127,.18));border-radius:6px}
      .dsh-i2-endpoint-head{display:flex;align-items:center;gap:6px}
      .dsh-i2-endpoint-index{min-width:1.4em;text-align:center;opacity:.6}
      .dsh-i2-endpoint-head .dsh-i2-input{flex:1}
      .dsh-i2-endpoint-key{display:grid;grid-template-columns:2fr 1fr auto;gap:6px;align-items:center}
      ...
      .dsh-i2s-presets{display:flex;flex-direction:column;gap:7px;border:1px solid var(--dsw-alias-border-l1);border-radius:10px;background:color-mix(in srgb,var(--dsw-alias-brand-primary) 3%,var(--dsw-alias-bg-layer-1));padding:9px 12px}
      .dsh-i2s-preset-head{display:flex;align-items:baseline;justify-content:space-between;gap:10px;flex-wrap:wrap}
      .dsh-i2s-preset-row{display:flex;align-items:center;gap:6px;flex-wrap:wrap}
      .dsh-i2s-select{box-sizing:border-box;min-width:0;flex:1 1 260px;height:30px;padding:0 8px;border:1px solid var(--dsw-alias-border-l2);border-radius:6px;background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);font:inherit;font-size:12px}
      .dsh-i2s-select:focus{outline:none;border-color:var(--dsw-alias-brand-primary)}
      .dsh-i2s-preset-save{display:flex;align-items:center;gap:6px;flex-wrap:wrap}
      .dsh-i2s-preset-save .dsh-i2s-input{flex:1 1 220px;height:30px}
      .dsh-i2s-var-box{display:flex;flex-direction:column;gap:6px;border-top:1px dashed var(--dsw-alias-border-l2);padding-top:8px}
      .dsh-i2s-var-row{display:flex;align-items:center;gap:8px}
      .dsh-i2s-var-row .dsh-i2s-param-label{width:auto;flex:none;font-family:ui-monospace,SFMono-Regular,Consolas,monospace;color:var(--dsw-alias-brand-primary)}
      .dsh-i2s-var-actions{display:flex;gap:6px;justify-content:flex-end}
      .dsh-i2s-preview{display:flex;flex-direction:column;gap:6px}
    `
